...
export default class MedoozePlayer {
  ...
  createPlayer(config: PlayerConfig) {
    ...
    this.streamPromise = new Promise((resolve, reject) => {
      (async () => {
        try {
          const pcc = await this.createPeerConnection(this.client, resolve, this.id);
          this.pcc = pcc;
  ...
  async createPeerConnection(cli: MediaServerClient, resolve: (stream: MediaStream) => void, camId: string) {
