...
export class PMSConnector {
  ...
  serverConnect(settings: PMSConnectorSettings) {
    const { host, httpOnly = true, token, port = 8000, isCloudServer, retries, isReconnecting } = settings;
      ...
      try {
        ...
        this.ws.onclose = () => {
          ...
          if (this.settings.retries > 0) {
            ...
            resolve(`Timeout for PMS: ${this.host} - To bypass SSL verification \n => ${this.api}`);
          }
        };
      } catch (error) {
  ...
  async checkBuildVersion(): Promise<boolean> {
    try {
      ...
      if (compareMinorVersions(pmsBuild.version, clientBuild.version) === false) {
        warning(
      ...
      warning(
        `Could not read PMS-version from /config/version endpoint. Upgrade PMS server to ${this.requiredVersion} or higher`,
      );
      console.error(error);
      return false;
    }
  ...
  reconnectWebSocket(settings: PMSConnectorSettings) {
    ...
    debug(`Reconnecting, attempts left: #${settings.retries} for ${settings.host}`);
    setTimeout(() => {
      this.serverConnect(settings);
    }, 3000);
  }
