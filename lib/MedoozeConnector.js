import { snackbar, debug, success, error } from '@piscada/snackbar'
...
export class PMSConnector {
  ...
  serverConnect(settings) {
    ...
    this.majorMinor = this.extractMajorMinorVersion(clientInfo.version)
  ...
  extractMajorMinorVersion(versionString) {
  ...
  async checkBuildVersion() {
    try {
      ...
      if (pmsBuild.version !== clientBuild.version) {
        error(
          `PMS version ${pmsBuild.version} is not compatible with PMS-client version ${clientBuild.version}. Please the PMS to ${this.majorMinor} or higher.`
        )
      }
    } catch (error) {
      error(
        `Could not get PMS-version. Upgrade to ${this.majorMinor} or higher`
      )
      console.error(error)
    }
  }
