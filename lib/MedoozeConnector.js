...
export class PMSConnector {
  ...
  serverConnect(settings) {
    ...
    this.majorMinor = extractMajorMinorVersion(clientInfo.version)
  ...
  async checkBuildVersion() {
    try {
      ...
      warning(
        `Could not read PMS-version from /config/version endpoint. Upgrade PMS server to ${this.majorMinor} or higher`
      )
      console.error(error)
    }
  }
